...
class GraphConstructorA():
    ...
    def connection_calc(self, features, region_labels,graph_method, topk=7):
        ...
        if len(region_labels)>0:
            ...
            d = self.distance_calc(features, graph_method, coordinates) #within each region calculate distance between patches
            dist.append(d)
        else:
    ...
    def construct_graph(self, features, img_ids, graph_method,experiment_folder ):
        """
        ...
        dist, inds, connection_pairs = self.connection_calc(features,coordinates, graph_method, topk = topk)
