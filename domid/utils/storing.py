...
class Storing:
    def __init__(self, args):
        ...
        if not os.path.exists(os.path.join("./notebooks", self.experiment_name)):
            ...
            df = pd.DataFrame(
                columns=["epoch", "loss", "accuracy_y", "accuracy_d", "val_loss", "val_accuracy_y", "val_accuracy_d"]
            )
        ...
        loss_acc_df = pd.DataFrame(
            {
                ...
                "val_accuracy_y": acc_val_y,
                "val_accuracy_d": acc_val_d,
            },
            index=[epoch],
        )
            ...
            row = [
                {
                    ...
                    "lr": self.args.lr,
                    "train_acc_y": self.acc_y[-1],
                    "test_acc_y": self.val_acc_y[-1],
                    "train_acc_d": self.acc_d[-1],
                    "test_acc_d": self.val_acc_d[-1],
                    "R with scores train": self.r_scores_tr[-1],
                    "R with scores test": self.r_scores_te[-1],
                    "train_loss": self.loss[-1],
                    "test_loss": self.val_loss[-1].item(),
